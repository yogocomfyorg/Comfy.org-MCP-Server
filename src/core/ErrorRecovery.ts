...
export class ErrorRecovery extends EventEmitter {
  ...
  private async executeStrategy(context: ErrorContext, strategy: RecoveryStrategy): Promise<boolean> {
      ...
      try {
        ...
        if (attempt < strategy.maxAttempts) {
          const delay = Math.min(
            strategy.delay * Math.pow(strategy.backoffMultiplier, attempt - 1),
            strategy.maxDelay
          );
  ...
  private isRecoverable(error: Error, severity: ErrorContext['severity']): boolean {
  ...
  public getActiveRecoveries(): RecoveryAttempt[] {
