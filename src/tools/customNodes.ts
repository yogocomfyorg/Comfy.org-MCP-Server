...
// Helper functions
async function executeCommand(command: string, cwd?: string, timeout = 60000): Promise<CommandResult> {
...
function extractNodeNameFromUrl(url: string): string {
  // Extract repository name from git URL
  const match = url.match(/\/([^\/]+?)(?:\.git)?$/);
  return match && match[1] ? match[1] : 'unknown-node';
}

async function parseReadmeForDependencies(readmePath: string): Promise<string[]> {
  try {
...
async function parseRequirementsFile(requirementsPath: string): Promise<string[]> {
  try {
...
async function detectPythonEnvironment(comfyuiPath: string): Promise<string> {
...
async function installDependencies(dependencies: string[], pythonPath: string, cwd: string): Promise<{ success: boolean; errors: string[] }> {
  const errors: string[] = [];
...
export async function registerCustomNodeTools(server: McpServer): Promise<void> {

  // Install custom nodes
  server.tool(
    "install_customnodes",
    "Install custom node via git clone, parse README and requirements, and install dependencies",
    {
  ...
  server.tool(
    "list_custom_nodes",
    "List all installed custom nodes with their information",
    {
      comfyuiPath: z.string().describe("Path to ComfyUI installation directory"),
  ...
  server.tool(
    "update_custom_node",
    "Update an installed custom node by pulling latest changes from git",
    {
      comfyuiPath: z.string().describe("Path to ComfyUI installation directory"),
    ...
    async ({ comfyuiPath, nodeName, updateDependencies }) => {
      try {
        ...
        let dependencyResult: { success: boolean; errors: string[] } = { success: true, errors: [] };
  ...
  server.tool(
    "remove_custom_node",
    "Remove an installed custom node completely",
    {
      comfyuiPath: z.string().describe("Path to ComfyUI installation directory"),
  ...
  server.tool(
    "check_custom_node_status",
    "Check the status and health of a specific custom node",
    {
      comfyuiPath: z.string().describe("Path to ComfyUI installation directory"),
