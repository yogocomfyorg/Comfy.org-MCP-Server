...
// DGM Integration for recursive improvement
interface DGMIntegration {
  logActivity(activity: string, success: boolean, errorMessage?: string): Promise<void>;
  analyzePerformance(): Promise<any>;
  suggestImprovements(): Promise<any>;
  runRecursiveImprovement(): Promise<any>;
}

// Types for image routing
...
class ImageDetector {
  ...
  private supportedFormats = ['.png', '.jpg', '.jpeg', '.webp', '.bmp', '.tiff'];
  private cache: Map<string, ImageMetadata[]> = new Map();
  private cacheTimeout = 30000; // 30 seconds cache
  private dgm: DGMIntegration;
  ...
  private createDGMIntegration(): DGMIntegration {
    return {
      async logActivity(activity: string, success: boolean, errorMessage?: string): Promise<void> {
        console.log(chalk.blue(`📊 DGM Activity: ${activity} - ${success ? 'SUCCESS' : 'FAILED'}`));
        if (errorMessage) console.log(chalk.red(`   Error: ${errorMessage}`));
      },
      async analyzePerformance(): Promise<any> {
        return { performance: 'analyzed', timestamp: Date.now() };
      },
      async suggestImprovements(): Promise<any> {
        return { improvements: ['cache optimization', 'error handling'], timestamp: Date.now() };
      },
      async runRecursiveImprovement(): Promise<any> {
        return { improvement: 'applied', timestamp: Date.now() };
      }
    };
  }

  private detectComfyUIDirectories(customPath?: string): string[] {
    const directories: string[] = [];

    // If custom path provided, prioritize it
    if (customPath) {
      directories.push(
        path.join(customPath, 'output'),
        path.join(customPath, 'temp'),
        path.join(customPath, 'models', 'output'),
        path.join(customPath, 'user', 'default', 'output')
      );
    }
      ...
      try {
        if (fs.existsSync(fullPath)) {
          ...
          if (hasMainPy || hasComfyUIDir) {
            ...
            directories.push(
              path.join(fullPath, 'output'),
              path.join(fullPath, 'temp'),
              path.join(fullPath, 'models', 'output'),
              path.join(fullPath, 'user', 'default', 'output')
            );
          }
        }
      } catch (error) {
  ...
  async getLatestImages(count: number = 5, filterPattern?: string): Promise<ImageMetadata[]> {
  ...
  private extractPrefix(filename: string): string {
    // Extract prefix from filename (everything before the first underscore and number)
    const match = filename.match(/^(.+?)_\d+/);
    return match?.[1] || filename.replace(/\.[^.]+$/, '');
  }

  private detectWorkflowSource(filePath: string): string {
    // Enhanced workflow source detection with more patterns
    const fullPath = filePath.toLowerCase();
    const filename = path.basename(filePath).toLowerCase();

    // Path-based detection
    if (fullPath.includes('/flux/') || fullPath.includes('\\flux\\')) return 'flux';
    if (fullPath.includes('/upscale/') || fullPath.includes('\\upscale\\') || fullPath.includes('upscaled_output')) return 'upscaling';
    if (fullPath.includes('/temp/') || fullPath.includes('\\temp\\')) return 'preview';
    if (fullPath.includes('/controlnet/') || fullPath.includes('\\controlnet\\')) return 'controlnet';
    if (fullPath.includes('/inpaint/') || fullPath.includes('\\inpaint\\')) return 'inpaint';

    // Filename pattern detection
    if (filename.includes('porsche') || filename.includes('car') || filename.includes('automotive')) return 'automotive';
    if (filename.includes('mint_green') || filename.includes('text2img')) return 'text2img';
    if (filename.includes('upscaled') || filename.includes('4x') || filename.includes('2x')) return 'upscaling';
    if (filename.includes('controlnet') || filename.includes('canny') || filename.includes('depth')) return 'controlnet';
    if (filename.includes('inpaint') || filename.includes('mask')) return 'inpaint';
    if (filename.includes('img2img')) return 'img2img';
    if (filename.includes('flux') || filename.includes('dev') || filename.includes('schnell')) return 'flux';

    // Prefix-based detection
    const prefix = this.extractPrefix(filename);
    if (prefix.includes('black_porsche') || prefix.includes('mint_green_porsche')) return 'automotive';
    if (prefix.includes('upscaled') || prefix.includes('enhanced')) return 'upscaling';
...
class WorkflowAnalyzer {
  private nodeTypeCache: Map<string, WorkflowImageNode[]> = new Map();

  analyzeWorkflow(workflow: any): WorkflowImageNode[] {
    ...
    const imageNodes: WorkflowImageNode[] = [];
    ...
    for (const [nodeId, node] of Object.entries(workflow)) {
      ...
      if (this.isImageInputNode(nodeData)) {
        const imageNode: WorkflowImageNode = {
          nodeId,
  ...
  private hashWorkflow(workflow: any): string {
    ...
    const nodeTypes = Object.values(workflow).map((node: any) => node.class_type).sort();
  ...
  private isImageInputNode(node: any): boolean {
  ...
  private hasImageInTitle(title?: string): boolean {
  ...
  private hasImageInput(inputs: any): boolean {
  ...
  private getImageInputName(node: any): string {
  ...
  private getAcceptedFormats(_nodeType: string): string[] {
    // Most ComfyUI nodes accept these formats
    return ['.png', '.jpg', '.jpeg', '.webp', '.bmp', '.tiff'];
  }

  private isRequiredInput(node: any): boolean {
...
async function waitForQueueCompletion(baseUrl: string = 'http://127.0.0.1:8188', timeoutMs: number = 120000): Promise<boolean> {
  const startTime = Date.now();
...
async function waitForImageGeneration(delayMs: number = 45000): Promise<void> {
...
async function executeWorkflow(workflow: any, baseUrl: string = 'http://127.0.0.1:8188'): Promise<any> {
...
class ImageRouter {
  ...
  private createDGMIntegration(): DGMIntegration {
    return {
      async logActivity(activity: string, success: boolean, errorMessage?: string) {
        try {
  ...
  async routeLatestImage(
    ...
    try {
      ...
      // Get latest images
      const latestImages = await this.detector.getLatestImages(
      ...
      const routingDecisions = this.makeRoutingDecisions(
        latestImages, 
        imageNodes, 
        options
      );
      ...
      const modifiedWorkflow = this.applyRoutingDecisions(
        workflow,
        routingDecisions
      );
  ...
  private calculateConfidence(
    ...
    if (node.nodeType === 'LoadImage') confidence += 0.3;
    if (image.workflowSource !== 'unknown') confidence += 0.2;
    if (options.workflowType && image.workflowSource === options.workflowType) confidence += 0.3;
   ...
   */
  private createAnnotatedImagePath(imagePath: string): string {
      ...
      if (node && node.inputs) {
        ...
        if (!this.validateImagePath(annotatedPath)) {
          console.log(chalk.yellow(
            `⚠ Warning: Image path may not be properly annotated: ${annotatedPath}`
          ));
        }
        ...
        console.log(chalk.green(
          `✓ Routed ${decision.sourceImage.filename} to node ${decision.targetNode.nodeId} using [output] annotation`
        ));
        console.log(chalk.blue(
          `  Image path: ${annotatedPath}`
        ));
        console.log(chalk.gray(
          `  Source: ${decision.sourceImage.fullPath}`
        ));
      }
    }
   ...
   */
  async testImageRouting(imagePath: string): Promise<{
    success: boolean;
...
export function registerImageRoutingTools(server: McpServer) {
  const router = new ImageRouter();

  // Enhanced fetch latest image tool with DGM integration
  server.tool(
    "fetch_latest_image",
    "Dynamically fetch the latest generated image and route it to the appropriate workflow node with enhanced path detection and DGM integration",
    {
    ...
    async ({
      ...
      try {
        ...
        if (filterPattern) routingOptions.filterPattern = filterPattern;
        if (preferredNodeTypes && preferredNodeTypes.length > 0) routingOptions.preferredNodeTypes = preferredNodeTypes;
        if (workflowType) routingOptions.workflowType = workflowType;
        ...
        const routingInfo = result.routingDecisions.map((decision, index) =>
          `📸 Route ${index + 1}: ${decision.sourceImage.filename}\n` +
          ...
          `   📏 Size: ${(decision.sourceImage.size / 1024).toFixed(1)} KB`
        ).join('\n\n');
  ...
  server.tool(
    "analyze_output_images",
    "Analyze generated images in output directories with metadata extraction",
    {
  ...
  server.tool(
    ...
    "Check if a workflow is compatible with available images and suggest routing options",
    {
      workflowFile: z.string().describe("Name of the workflow file (without .json extension)"),
  ...
  server.tool(
    "batch_route_images",
    "Route multiple images to different workflows or nodes in batch",
    {
    ...
    async ({ routingConfig, dryRun }) => {
      try {
        ...
        for (const config of routingConfig) {
          ...
          const workflow = await fs.readJson(workflowPath);
          const routingOptions: any = { imageCount: config.count };
          if (config.imagePattern) routingOptions.filterPattern = config.imagePattern;
  ...
  server.tool(
    "smart_route_image",
    "Advanced image routing with intelligent node selection and confidence scoring",
    {
      workflowFile: z.string().describe("Name of the workflow file (without .json extension)"),
    ...
    async ({ workflowFile, imagePattern, confidenceThreshold, learningMode, dryRun }) => {
      try {
        ...
        if (imagePattern) routingOptions.filterPattern = imagePattern;
        ...
        const highConfidenceDecisions = result.routingDecisions.filter(
          decision => decision.confidence >= confidenceThreshold
        );
        ...
        const routingInfo = highConfidenceDecisions.map(decision =>
          `🎯 ${decision.sourceImage.filename} → Node ${decision.targetNode.nodeId}\n` +
          `   Confidence: ${(decision.confidence * 100).toFixed(1)}% ✨\n` +
          `   Smart Reasoning: ${decision.reasoning}`
        ).join('\n\n');
  ...
  server.tool(
    "orchestrate_workflow_with_images",
    "Orchestrate multiple workflows with automatic image routing between them",
    {
