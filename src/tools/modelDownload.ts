...
// Helper functions
async function downloadFile(url: string, outputPath: string, onProgress?: (progress: DownloadProgress) => void): Promise<number> {
  let retries = 3;
...
async function getHuggingFaceModelInfo(modelId: string, filename?: string): Promise<ModelInfo> {
  ...
  let targetFile = files.find((f: any) => f.rfilename === filename);
  if (!targetFile && files.length > 0) {
    // Default to first .safetensors or .ckpt file
    targetFile = files.find((f: any) => 
      f.rfilename.endsWith('.safetensors') || f.rfilename.endsWith('.ckpt')
    ) || files[0];
  }
...
function detectModelType(filename: string): string {
  ...
  if (lower.includes('upscale')) return 'upscale';
  if (lower.includes('clip')) return 'clip';
  if (lower.includes('unet')) return 'unet';
  return 'checkpoint';
}

function getModelDirectory(modelType: string, baseDir: string): string {
...
export async function registerModelDownloadTools(server: McpServer): Promise<void> {
