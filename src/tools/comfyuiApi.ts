...
let globalOrchestrator: ServerOrchestrator | null = null;

export function setOrchestrator(orchestrator: ServerOrchestrator): void {
  globalOrchestrator = orchestrator;
  console.error(chalk.blue("🎭 ComfyUI API tools connected to orchestrator"));
}

// Simplified - removed unused complex functions
...
// Helper functions
export async function makeComfyUIRequest(endpoint: string, baseUrl = 'http://127.0.0.1:8188', method = 'GET', data?: any) {
  try {
...
export async function registerComfyUIApiTools(server: McpServer): Promise<void> {
  // Get server status
  server.tool(
    "get_server_status",
    "Check ComfyUI server status and health",
    {
        baseUrl: z.string().default("http://127.0.0.1:8188").describe("ComfyUI server base URL")
    },
    async ({ baseUrl }) => {
      return await executeToolOperation(
        globalOrchestrator,
        'ComfyUIApi',
        'get_server_status',
        { baseUrl },
        ...
        const queueStatus: QueueStatus = queueData;
        const serverInfo: ComfyUIServerInfo = {
          status: 'running',
          url: baseUrl,
  ...
  server.tool(
    "queue_workflow",
    "Add a workflow to the ComfyUI processing queue",
    {
  ...
  server.tool(
    ...
    async ({ baseUrl }) => {
      try {
        const queueData: QueueStatus = await makeComfyUIRequest('/queue', baseUrl);
  ...
  server.tool(
    "clear_queue",
    "Clear all pending items from the queue and optionally kill running processes",
    {
        baseUrl: z.string().default("http://127.0.0.1:8188").describe("ComfyUI server base URL"),
  ...
  async function cleanupPorts(results: string[], execAsync: any): Promise<void> {
  ...
  server.tool(
    "kill_comfyui_processes",
    "Force kill all ComfyUI processes and optionally active terminals (useful when server is unresponsive)",
    {
  ...
  server.tool(
    "cleanup_comfyui_ports",
    "Clean up all ports used by ComfyUI and related services to resolve connection issues",
    {
