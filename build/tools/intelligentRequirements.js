...
async function parseReadmeIntelligently(readmePath) {
    try {
        ...
        const patterns = [
            // Python patterns
            {
                ...
                packageManager: 'pip',
                confidence: 0.9,
                extractor: (match) => match.replace(/pip install\s+/i, '').split(/\s+/).filter(pkg => pkg && !pkg.startsWith('-'))
            },
            {
                regex: /conda install\s+([^\n\r`]+)/gi,
                packageManager: 'conda',
                confidence: 0.9,
                extractor: (match) => match.replace(/conda install\s+/i, '').split(/\s+/).filter(pkg => pkg && !pkg.startsWith('-'))
            },
            {
                regex: /poetry add\s+([^\n\r`]+)/gi,
                packageManager: 'poetry',
                confidence: 0.9,
                extractor: (match) => match.replace(/poetry add\s+/i, '').split(/\s+/).filter(pkg => pkg && !pkg.startsWith('-'))
            },
            // Node.js patterns
            {
                regex: /npm install\s+([^\n\r`]+)/gi,
                packageManager: 'npm',
                confidence: 0.9,
                extractor: (match) => match.replace(/npm install\s+/i, '').split(/\s+/).filter(pkg => pkg && !pkg.startsWith('-'))
            },
            {
                regex: /yarn add\s+([^\n\r`]+)/gi,
                packageManager: 'yarn',
                confidence: 0.9,
                extractor: (match) => match.replace(/yarn add\s+/i, '').split(/\s+/).filter(pkg => pkg && !pkg.startsWith('-'))
            },
            // Generic installation patterns
            {
                ...
                confidence: 0.5,
                extractor: (match) => [match.replace(/install\s+/i, '').trim()]
            },
...
async function detectRequirementFiles(projectPath) {
    const files = [];
    const fileChecks = [
        {
...
async function parsePyprojectToml(filePath) {
    const content = await fs.readFile(filePath, 'utf-8');
    const dependencies = [];
...
async function parseSetupPy(filePath) {
    const content = await fs.readFile(filePath, 'utf-8');
    const dependencies = [];
...
async function parsePipfile(filePath) {
    const content = await fs.readFile(filePath, 'utf-8');
    const dependencies = [];
...
async function parseEnvironmentYml(filePath) {
    const content = await fs.readFile(filePath, 'utf-8');
    const dependencies = [];
...
function createInstallationStrategy(environment, requirementFiles, readmeInstructions) {
    const strategy = {
    ...
    if (relevantInstructions.length > 0) {
        ...
        const pythonFiles = requirementFiles.filter(f => ['requirements.txt', 'pyproject.toml', 'setup.py'].includes(f.type));
            ...
            if (reqFile) {
                switch (environment.type) {
                    case 'venv':
                        ...
                        strategy.fallbackCommands = reqFile.dependencies.map(dep => `"${environment.pythonPath}" -m pip install "${dep}"`);
                        break;
                    case 'conda':
...
async function executeInstallation(strategy, projectPath) {
    const result = {
        success: false,
        output: '',
        errors: [],
...
export async function registerIntelligentRequirementsTools(server) {
    server.tool("install_requirements", "Intelligently analyze and install requirements from GitHub repositories by reading README files and detecting environment setup", {
...
//# sourceMappingURL=intelligentRequirements.js.map