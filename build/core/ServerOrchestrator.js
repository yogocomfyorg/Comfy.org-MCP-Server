...
export class ServerOrchestrator extends EventEmitter {
    ...
    async executeWithRecovery(operation, toolName, executor, metadata = {}) {
        ...
        try {
            ...
            if (this.stateManager) {
                ...
                this.stateManager.recordError(error, false);
            }
            // Attempt recovery if enabled
            if (this.errorRecovery) {
                const context = this.errorRecovery.createErrorContext(operation, toolName, error, 'medium', metadata);
    ...
    async handleCriticalFailure(type, data) {
    ...
    async handleProcessFailure(name, _code, _signal) {
    ...
    async handleProcessError(name, error) {
        if (this.errorRecovery) {
            const context = this.errorRecovery.createErrorContext('process_management', 'ProcessManager', error, 'high', { processName: name });
            await this.errorRecovery.handleError(context);
        }
    }
    async handleRecoveryFailure(context) {
...
//# sourceMappingURL=ServerOrchestrator.js.map